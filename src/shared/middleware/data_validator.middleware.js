...
module.exports = {
  validateRequestData(schema, data = {}) {
    return async (req, res, next) => {
      try {
        if (!schema)
          return next(APIError.badRequest('Schema name is required'));
        if (schema === 'ZAccountSchema') {
          ...
          if (isStrongPassword(req.body.password) === false)
            return next(APIError.badRequest('Password is weak'));
        }
  ...
  allowedRoles(roles) {
    return async (req, res, next) => {
      try {
        if (!roles.includes(req.userRole.toLowerCase()))
          return next(
            APIError.unauthorized("You don't have required permission")
          );
        next();
      } catch (error) {
        next(error);
  ...
  notAllowedAccount(type) {
    return async (req, res, next) => {
      try {
        if (type.toLowerCase() === req.userType.toLowerCase())
          return next(
            APIError.unauthorized("You don't have required permission")
          );
        next();
  ...
  renameZodSchema(schema) {
    return async (req, res, next) => {
      try {
        ...
        if (!schema)
          return next(APIError.badRequest('Zod Schema name is required'));
        req.schema = rename;
        next();
      } catch (error) {
        next(error);
